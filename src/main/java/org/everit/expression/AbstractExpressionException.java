 ...
 */
public abstract class AbstractExpressionException extends RuntimeException {
    ...
    public AbstractExpressionException(final String message, final char[] expr, final int cursor) {
        super(message);
        this.expr = expr;
        this.cursor = cursor;
    }

    public AbstractExpressionException(final String message, final char[] expr, final int cursor, final Throwable e) {
        super(message, e);
        this.expr = expr;
        this.cursor = cursor;
    }
    ...
    private CharSequence showCodeNearError(final char[] expr, final int cursor) {
        ...
        if (cursor < end) {
            matchStart = cursor;
            if (matchStart > 0) {
                while (matchStart > 0 && expr[matchStart - 1] <= ' ') {
                    matchStart--;
                }
            }
